 ...
 */
metadata {
	definition (name: "Forecast High Temp", namespace: "rodneyrowen", author: "RodneyRowen") {
	...
	tiles {
        multiAttributeTile(name:"temperature", type:"generic", width:6, height:4, canChangeIcon: false) {
            tileAttribute("device.forecastHigh", key: "PRIMARY_CONTROL") {
            ...
            tileAttribute("device.forecastLow", key: "SECONDARY_CONTROL") {
                attributeState("default", label:'Low: ${currentValue}°')
            }
        }    

		standardTile("weatherIcon", "device.forecastIcon", width: 2, height: 2, decoration: "flat") {
		...
		valueTile("high", "device.forecastHigh", width: 2, height: 2) {
			state("forecastHigh", label:'High ${currentValue}', unit:"dF",
				backgroundColors:[
		...
		valueTile("low", "device.forecastLow", width: 2, height: 2) {
			state("forecastLow", label:'Low ${currentValue}', unit:"dF",
				backgroundColors:[
					[value: 31, color: "#153591"],
					[value: 44, color: "#1e9cbb"],
					[value: 59, color: "#90d2a7"],
		...
		valueTile("current", "device.temperature", width: 2, height: 2) {
			state("temperature", label:'Current ${currentValue}', unit:"dF",
				backgroundColors:[
					[value: 31, color: "#153591"],
					[value: 44, color: "#1e9cbb"],
					[value: 59, color: "#90d2a7"],
		...
		main(["forecastHigh"])
		details(["temperature", "weatherIcon", "current", "low", "percentPrecip", "humidity", "feelsLike", "wind", "weather", "city", "ultravioletIndex", "alert", "refresh", "rise", "set", "light", "lastUpdate"])}
}
...
def poll() {
	...
	if (obs) {
		...
		if(getTemperatureScale() == "C") {
			send(name: "current", value: Math.round(obs.temp_c), unit: "C")
			send(name: "feelsLike", value: Math.round(obs.feelslike_c as Double), unit: "C")
		} else {
			send(name: "current", value: Math.round(obs.temp_f), unit: "F")
		...
		if (f1) {
            ...
            if(getTemperatureScale() == "C") {
				send(name: "temperature", value: Math.round(f1[0].high.celsius), unit: "C")
            	send(name: "forecastHigh", value: f1[0].high.celsius as String, unit: "C")
            	send(name: "forecastLow", value: f1[0].low.celsius as String, unit: "C")
            } else {
				send(name: "temperature", value: Math.round(f1[0].high.fahrenheit), unit: "F")
            	send(name: "forecastHigh", value: f1[0].high.fahrenheit as String, unit: "F")
            	send(name: "forecastLow", value: f1[0].low.fahrenheit as String, unit: "F")
            }
		}
		else {
